...
export const LUMEN_VERSION = "0.1.10";
...
export default function lumenExtension(pi: ExtensionAPI) {
	...
	const generateVisualTool = defineTool({
		...
		description:
			'Generate a single-file HTML visualization. Wired routes: mermaid (flowchart/sequence/er/state/mermaid_custom — pass mermaid source as `content`); fgraph diagram (type:"diagram" — structured FgraphContent object, topologies sequence / layered / linear-flow / radial-hub / radial-ring / lane-swim / deployment-tiers / machine-clusters / state / gantt / pie / er / dep-graph / dual-cluster / system-architecture); chart (type:"chart" — structured ChartContent object, types bar / pie / line / table). Other types (architecture, slides, galleries, guides, recaps, fact-checks) throw NotImplementedError; invoke the matching lumen-* skill directly. Opens result in browser.',
		...
		promptGuidelines: [
			...
			'Fgraph diagram (`type:"diagram"`): pass a structured object as `content` with a `topology` discriminator. Supported topologies in v0.2: "sequence" (participants + messages), "layered" (layers of nodes), "linear-flow" (left→right pipeline of stages), "radial-hub" (hub + spokes at compass positions), "radial-ring" (peer cluster), "lane-swim" (lanes × rows pipeline), "deployment-tiers" (multi-tier deployment), "machine-clusters" (cross-host deployment), "state" (FSM / state machine), "gantt" (schedule / timeline bars), "pie" (composition / proportion), "er" (relational schema), "dep-graph" (dependency DAG), "dual-cluster" (side-by-side comparison), "system-architecture" (≥15 components, full-system). See FgraphContent in src/templates/diagram/schemas.ts.',
